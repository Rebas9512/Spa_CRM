...
interface ServiceOverview {
  serviceBreakdown: { foot: number; body: number; combo: number; chair: number; head: number; total: number }
...
const SERVICE_COLORS = { foot: '#0F766E', body: '#F59E0B', combo: '#6366F1', chair: '#EC4899', head: '#06B6D4' }
              ...
              {svc && (
                <div className="flex items-end gap-6 justify-center" style={{ height: 100 }}>
                  {[
                    ...
                    { key: 'Head (H)', val: svc.serviceBreakdown.head ?? 0, color: SERVICE_COLORS.head },
                  ].map((s) => {
                    const max = Math.max(svc.serviceBreakdown.foot, svc.serviceBreakdown.body, svc.serviceBreakdown.combo, svc.serviceBreakdown.chair, svc.serviceBreakdown.head ?? 0, 1)
