...
admin.get('/analytics/service-overview', async (c) => {
  ...
  `).bind(adminId, year).first<{ foot: number; body: number; combo: number; chair: number; head: number; total: number }>()
  ...
  return c.json({
    serviceBreakdown: {
      foot: fbc?.foot ?? 0, body: fbc?.body ?? 0, combo: fbc?.combo ?? 0, chair: fbc?.chair ?? 0, head: fbc?.head ?? 0, total: fbc?.total ?? 0,
    },
