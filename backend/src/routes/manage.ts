...
manage.get('/analytics/service-breakdown', async (c) => {
  ...
  // Match F/B/C/A/H from therapist_service_technique (case-insensitive first letter)
  const rows = await c.env.DB.prepare(`
    SELECT
      ...
      SUM(CASE WHEN UPPER(SUBSTR(TRIM(v.therapist_service_technique), 1, 1)) = 'H' THEN 1 ELSE 0 END) as head,
      COUNT(*) as total
    FROM visits v
    WHERE v.store_id = ? AND v.therapist_signed_at IS NOT NULL ${dateFilter}
  `).bind(session.storeId).first<{ foot: number; body: number; combo: number; chair: number; head: number; total: number }>()
...
manage.get('/analytics/therapist-ranking', async (c) => {
  ...
  const rows = await c.env.DB.prepare(`
    SELECT therapist_name, COUNT(*) as cnt,
      ...
      SUM(CASE WHEN UPPER(SUBSTR(TRIM(therapist_service_technique), 1, 1)) = 'H' THEN 1 ELSE 0 END) as head
    FROM visits
    WHERE store_id = ? AND therapist_signed_at IS NOT NULL
      ${dateFilter}
    GROUP BY therapist_name
    ORDER BY cnt DESC
  `).bind(session.storeId).all<{ therapist_name: string; cnt: number; foot: number; body: number; combo: number; chair: number; head: number }>()
  ...
  interface Cluster { canonical: string; count: number; foot: number; body: number; combo: number; chair: number; head: number; variants: string[] }
...
manage.get('/analytics/top-customers', async (c) => {
  ...
  `).bind(session.storeId, year).all<{
    ...
    visit_count: number; foot: number; body: number; combo: number; chair: number; head: number
  }>()

  return c.json({
    customers: (rows.results || []).map((r) => ({
      id: r.id, name: `${r.first_name} ${r.last_name}`, phone: r.phone,
      visitCount: r.visit_count, foot: r.foot, body: r.body, combo: r.combo, chair: r.chair, head: r.head,
    })),
  })
})
